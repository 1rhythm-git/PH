...
• 빨간 운동화는 이동속도 +20%, 10초 지속
• 날개달린 신발은 이동속도 +50%, 12초 지속
...
우선순위 후보:
1. 버프 남은 시간 UI / 버프 아이콘 표시
2. Enemy 다중 배치 / 층별 배치 규칙
3. EnemyTrail 예고선
...
현재 권장 다음 작업:
• 신발 아이템에 지속시간 버프가 생겼으므로, 다음은 TopUI에 현재 활성 버프와 남은 시간을 표시하는 구조를 검토한다.
• Enemy 기본 이동 규칙이 생겼으므로 이후 여러 Enemy를 라인별/층별로 배치하는 구조를 만든다.
• EnemyTrail 예고선을 붙이면 실제 회피 판단 테스트가 가능해진다.
